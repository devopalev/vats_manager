...
class EditUsersMenu:
    ...
    def _edit_users_create(cls, update, context):
        ...
        update.callback_query.message.edit_text(
            text='Введите логин telegram нового пользователя, например yngphenix',
            reply_markup=InlineKeyboardMarkup(keyboard))
    ...
    def _edit_user_param(cls, update, context):
        ...
        if query[2] == 'username_tg':
            text = f'Напишите логин telegram, например yngphenix'
            ...
            text = f'Напишите логин пользователя ВАТС, если необходимо указать ' \
                   ...
                   f'например m.opalev;asterisk_opalev;opalev-g\n' \
            ...
            text = f'Напишите ФИО, например Опалев Максим Сергеевич'
            save_and_edit_keboard(text)
            return cls._edit_users_mes

        elif query[2] == 'username_skuf':
            text = f'Напишите логин СКУФ, например maksim.opalev'
            save_and_edit_keboard(text)
            return cls._edit_users_mes
...
class NttmMenu:
    ...
    def _join_ki_to_si_menu(cls, update, context):
        ...
        if not services.nttm.check_client():
            context.bot.answer_callback_query(update.callback_query.id, text="Модуль NTTM е подключен!")
        ...
        if si:
            ...
            text = "Выбирите к какому сетевому INC привязать тикет"
            update.callback_query.message.edit_text(text=text, reply_markup=keyboard, parse_mode='HTML')
    ...
    def _inc_accept(cls, update, context, thread_start=False):
        ...
        if thread_start is False:
            ...
            # Проверка наличия логина скуф в БД
                ...
                params.update({'show_alert': True, 'text': "Неизвестен ваш логин СКУФ. Обратитесь к @yngphenix"})
...
class SkufCreateINC:
    ...
    def _select_initiator(cls, update, context):
        ...
        # Структура сообщения
        text = 'Выбирите инициатора инцидента на клавиатуре'
