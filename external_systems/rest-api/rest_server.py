...
TOKENS = ['0E6C03C405E6525C64C184258C44EC99', '58D26FE10F6F518FE4E5340DFFCC7381', 'A4C6BBEE3FC5E7961F6BE4DCC9773901']
app = flask.Flask(__name__)


def sql_request(sql, data_python=False):
    connect = sqlite3.connect("/home/maksim/vats_manager/bot.db", detect_types=sqlite3.PARSE_DECLTYPES)
    ...
    if data_python:
        ...
        cursor.execute(sql)
    if sql[:6] == 'SELECT' or sql[:6] == 'PRAGMA':
...
def set_analytics_nttm_tickets_vendor():
    ...
    sql = f"UPDATE analytics_nttm SET value = ? WHERE type = 'tickets_vendor'"
    sql_request(sql, (data_json, ))
...
def call_events():
    ...
    if event:
        if event['type'] == 'outbound' or event['type'] == 'incoming':
            try:
                connect = sqlite3.connect("/home/maksim/vats_manager/bot.db")
                cursor = connect.cursor()
