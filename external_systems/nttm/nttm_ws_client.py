...
class ConnectorNTTM:
    def __init__(self, loop):
        ...
        if TEST:
            self._url = 'http://10.42.110.22'
            # self._url = 'http://10.28.97.112'
        else:
            self._url = 'http://192.168.68.56'
    ...
    async def _binding_network_inc(self, task_id, inc_si, inc_ki, skip_mrf='МРФ Центр'):
        ...
        try:
            ...
            if not error and skip:
                ...
                if not error:
                    ...
                    if not rule:
                        ...
                        comment = "Привязка ТТ к сетевому инциденту средствами vats manager bot"
                ...
                if not error:
                    code, tasks = await get_tasks(inc_ki, new_task=True, old_task_id_=task_end['id'])
                    if code == 200:
                        task_end = tasks[-1]
                    else:
            ...
            if not error:
                rule = rule_ids.get('Связать с СИ')
                unit = unit_ids.get('Решение сетевого ТТ')
...
async def main():
    ...
    while True:
        try:
            async with websockets.connect("ws://193.228.110.92:80") as ws:
                # Авторизация
