    ...
    def create_task(self, task: dict):
        ...
        with self._lock:
            if self._end_id < self._max_id:
                # Присваиваем заданию ID
                task_id = self._end_id + 1
                # Записываем последний ID
                self._end_id = task_id
            else:
                task_id = 1
                self._end_id = 1

            # Добавляем task id в задание
...
class ServerServices:
    ...
    def __init__(self, port):
        self._port = port
        self._TOKEN = '84700991B4357AD447D9A4D2F283BEDA'
        self._KEY = 'IFdgzXJiwQiWSfZlnDOm-E4db0vVl4EpGn60I8u4zzQ='
